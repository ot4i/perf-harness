...
public final class RequestorAsync extends MQJavaWorkerThread implements WorkerThread.Paceable {
    ...
    // Used to ensure we don't start to sending messages before the get thread is
    ...
    public boolean oneIteration() throws Exception {
        ...
        InFlightMessageDetails imd = getOrCreateInFlightMessageDetails(outMessage.messageId, putTime,
                System.currentTimeMillis() + expiryInMilliSeconds);
    ...
    /**
     * Partner method for oneIteration(). Sits in a loop waiting for MQ messages to
     * arrive, at which point the correlID is used to check the map of in-flight
     * messages. If the entry is found, then the messages has arrived in time, the
     * entry is removed, and the statistics are updated using the receive time and
     * also the put time from the InFlightMessageDetails.
     */
    public void getMessages() throws Exception {
        ...
        while (!stopping || gotMessageOnLastIteration) {
            ...
            if (messageDetails == null) {
                incUnknownMessages();
                System.out.println(
                        "In RequestorAsync.getMessages - correlId " + messageIdHolder + " did not match (transacted "
                                + transacted + ") messageIdHolder.dataHashCode " + messageIdHolder.dataHashCode);
            } else {
    ...
    /**
     * Single thread for the whole process. Sits in a loop waiting for expiry times
     * to be reached, at which point the message ID is used to check the map of
     * in-flight messages. If the entry is found, then the messages has timed out,
     * because otherwise the get reply thread would have removed the entry when the
     * reply arrived.
     */
                ...
                while (!RequestorAsync.stopping) {
                    ...
                    if (imd.expiryZeroHour < System.currentTimeMillis()) {
                        ...
                        if (RequestorAsync.timeoutQueue.poll() != imd)
                            System.out.println("In TimeoutThread.run - imd.messageID " + imd.messageID
                                    + " picked up by someone else!");

                        // The normal case is this next call returns null because the message has been
                        // claimed by one of the GET threads, so if we actually manage to take the 
                        // in-flight object then it has definitely timed out.
                        RequestorAsync.InFlightMessageDetails messageDetails = RequestorAsync.messageIDsInFlight
                                .remove(imd.messageID);
                        if (messageDetails != null) {
    ...
    // Return queue for InFlightMessageDetails if the "reduce heap usage" option is
    // set to true
    ...
    /**
     * Wrapper method for InFlightMessageDetails creation. Checks the queue if in
     * reduced heap usage mode, and creates a new object if not found (or if not 
     * in rhu mode)
     */
    public InFlightMessageDetails getOrCreateInFlightMessageDetails(byte[] messageIDAsBytes, long putTime,
            long expiryZeroHour) {
